...
CONFIG="configs/experiments/v10_transunet_path_confirmation.yaml"
ROOT="results/v10_transunet_path_confirmation"
FORMAL_ROOT="$ROOT/formal_test"
LOCK="$ROOT/v10_protocol_lock.json"
LOG_ROOT="logs/v10_formal"
...
write_status "PROTOCOL_LOCK" "RUNNING" \
  "locking one path and 224 remaining path-outcome-naive patients"
if ! "$PYTHON_BIN" scripts/lock_v10_transunet_protocol.py \
...
scientific_status="$($PYTHON_BIN - "$FORMAL_ROOT/v10_status.json" <<'PY'
import json
import sys
from pathlib import Path
...
printf 'V10 pipeline complete: %s\n' "$scientific_status"
