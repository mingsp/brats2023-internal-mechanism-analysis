...
OUTPUT_ROOT="${OUTPUT_ROOT:-stage1_explanation_suite_20260517/results/e9_class_region_response_formal_n512}"
CASE_SET_CSV="${CASE_SET_CSV:-stage1_explanation_suite_20260517/results/common_case_sets/formal_n512_cases.csv}"
E4_ADJUSTED_CSV="${E4_ADJUSTED_CSV:-stage1_explanation_suite_20260517/results/e4_faithfulness_perturbation_formal_n512_allnodes_main/table_perturbation_random_adjusted.csv}"
...
"${PYTHON_BIN}" -u guide_unet/process_analysis/stage1_class_region_response_analysis.py \
  --model-spec "baseline=${BASELINE_CKPT}" "noskip_unet=${NOSKIP_CKPT}" \
  --data-dir "${DATA_DIR}" \
  --output-root "${OUTPUT_ROOT}" \
  --case-set-csv "${CASE_SET_CSV}" \
  --e4-adjusted-csv "${E4_ADJUSTED_CSV}" \
  --num-cases "${NUM_CASES}" \
