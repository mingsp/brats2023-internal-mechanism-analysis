...
OUTPUT_ROOT="${OUTPUT_ROOT:-stage1_explanation_suite_20260517/results/e5_feature_response_robustness}"
CASE_SET_CSV="${CASE_SET_CSV:-stage1_explanation_suite_20260517/results/common_case_sets/dev_n64_cases.csv}"
STAGE1_READOUT_CSV="${STAGE1_READOUT_CSV:-results/stage1_case_readout_alignment_n512/table_stage1_case_readout.csv}"
...
"${PYTHON_BIN}" -u guide_unet/process_analysis/stage1_feature_response_robustness.py \
  --model-spec "baseline=${BASELINE_CKPT}" "noskip_unet=${NOSKIP_CKPT}" \
  --data-dir "${DATA_DIR}" \
  --output-root "${OUTPUT_ROOT}" \
  --case-set-csv "${CASE_SET_CSV}" \
  --stage1-readout-csv "${STAGE1_READOUT_CSV}" \
  --num-cases "${NUM_CASES}" \
